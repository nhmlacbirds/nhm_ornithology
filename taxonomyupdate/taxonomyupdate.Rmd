...
# extract subspecies
translat$ssp_pet <- ifelse(translat$io_rank == "ssp", sub(".* ", "", translat$peters), "NA")
...
translat$ssp_clements <- ifelse(translat$io_rank == "ssp", sub(".* ", "", translat$clements), "NA")
...
check <- upd0[is.na(upd0$clements),] #2395 
...
# check those missing
check3 <- upd3[is.na(upd3$clements),] #2327 okay, fewer than before but slightly
...
# filter NA
check4 <- upd4[is.na(upd4$clements),] #1437 okay, fewer than before but still a lot 
